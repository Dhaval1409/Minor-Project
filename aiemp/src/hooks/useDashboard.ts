...
export function useDashboard() {
  ...
  const handleAddAppointment = async (form: any) => {
    // ◄ CHANGED: form now sends `services: string[]` (multi-service booking)
    // instead of a single `service: string`, so validate the array length.
    const hasServices = Array.isArray(form.services) && form.services.length > 0;
    if (!form.name || !form.phone || !hasServices || !form.date || !form.time) {
      setApptError('Fill in name, phone, at least one service, date, and time.');
      return;
    }
  ...
  // ◄ ADDED: mirrors handleCancelAppointment, but hits the PATCH /:id/complete
  // endpoint wired up in appointmentRoutes.ts (completeAppointment controller),
  // which marks status="completed" and fires the thank-you Telegram message.
  const handleCompleteAppointment = async (id: string) => {
    const activeBizId = businessId || localStorage.getItem('aria_business_id');
    try {
      const res = await fetch(`${API_BASE}/appointments/${id}/complete`, { method: 'PATCH' });
      const data = await res.json();
      if (!res.ok || data.success === false) {
      ...
      setApptError(err.message || 'Failed to complete appointment.');
    }
  };
