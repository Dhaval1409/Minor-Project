...
function normalizeServices(businessContext: any): string[] {
  // ◄ FIXED: previously this read the legacy `servicesProvided` string list
  // FIRST, so anything added/edited/deleted in the dashboard's Services tab
  // (which writes to the new `services` array) never reached the bot —
  // it kept quoting whatever was set at onboarding, forever.
  //
  // Now: prefer the new `services` array (objects with name/price/duration/
  // active), showing only services the owner has left active. Only fall
  // back to the legacy string list if the new array is empty (businesses
  // that haven't touched the Services tab yet).
  const richServices = businessContext?.services;
  if (Array.isArray(richServices) && richServices.length > 0) {
    return richServices
      .filter((s: any) => s && s.active !== false && s.name)
...
// ◄ ADDED: same as normalizeServices, but keeps price/duration so the
// "what services do you offer" reply can show them instead of bare names.
function getServiceOfferLines(businessContext: any): string[] {
  const richServices = businessContext?.services;
  if (Array.isArray(richServices) && richServices.length > 0) {
    return richServices
      .filter((s: any) => s && s.active !== false && s.name)
      .map((s: any) => {
        const price = typeof s.price === "number" && s.price > 0 ? ` — ₹${s.price}` : "";
        const duration = s.duration ? ` (${s.duration})` : "";
        return `${String(s.name).trim()}${price}${duration}`;
      });
  }
  // Legacy fallback — plain names, no price/duration available.
  return normalizeServices(businessContext);
}
...
const SERVICES_QUESTION_PATTERN =
  /\b(what|which)\b.*\b(service|services|product|products|offer|provide|menu|sell)\b|^(services|menu)$/i;

// ◄ ADDED: separate, narrower intent — only true price/cost questions match
// this. "what services do you offer" does NOT match, so plain browsing
// never shows numbers unprompted; only an explicit ask for price does.
const PRICE_QUESTION_PATTERN =
  /\b(price|prices|pricing|cost|costs|charge|charges|fee|fees|rate|rates|how much)\b|^price\s*list$/i;

function tryFastPathServicesAnswer(message: string, businessContext: any): AIResponse | null {
  const trimmed = message.trim();
  const asksServices = SERVICES_QUESTION_PATTERN.test(trimmed);
  const asksPrice = PRICE_QUESTION_PATTERN.test(trimmed);
  if (!asksServices && !asksPrice) return null;

  // Only reveal price/duration when the customer's own wording signals
  // they actually want it — plain "what do you offer" stays names-only.
  const services = asksPrice ? getServiceOfferLines(businessContext) : normalizeServices(businessContext);
