...
//   getAppointmentById,
//   deleteAppointment,
//   completeAppointment, // ◄ ADDED: handler that marks status=completed + sends thank-you Telegram message
...
// // 5. Mark an appointment as completed + send thank-you Telegram message
// //    (PATCH http://localhost:5000/appointments/:id/complete)
...
import {
  ...
  deleteAppointment,
  completeAppointment, // ◄ ADDED: handler that marks status=completed + sends thank-you Telegram message
  getAvailableSlots, // ◄ ADDED: returns full day's slots (booked + available) for chat/dashboard
} from "../controllers/appointmentController";
import BusinessModel from "../models/businessModel";
import { startIndividualShopBot } from "../config/botManager";
...
// 2b. List every slot for a day (booked + available) — used by dashboard/chat
//     (GET http://localhost:5000/appointments/slots?businessId=...&date=YYYY-MM-DD)
//     Placed BEFORE "/:id" so Express doesn't treat "slots" as an :id param.
