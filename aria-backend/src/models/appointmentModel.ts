...
export interface AIResponse {
  ...
  reply: string; // natural language reply to send back to the user
  missingFields?: string[];
  // ◄ ADDED: true only once the customer has explicitly said "yes" to a
  // proposed booking. intent === "book" with confirmed !== true means the
  // AI has extracted all the fields but is WAITING on confirmation — the
  // caller (botManager.ts) must NOT write to the database until this is true.
  confirmed?: boolean;
}
...
export const AppointmentModel = {
  ...
  /**
   * ◄ ADDED: tenant-scoped listing. Used by the authenticated
   * GET /appointments route so one business can never see another
   * business's bookings (findAll() above is unscoped and is now only
   * intended for internal/admin use).
   */
  async findAllByBusiness(businessId: string): Promise<Appointment[]> {
    const docs = await MongooseAppointmentModel
      .find({ businessId })
      .sort({ createdAt: -1 })
      .lean();
